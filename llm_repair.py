...
def repair_segments(
    ...
    source = {
        ...
        "canonical_terms": source_context.get("terms", []),
    ...
    for chunk_index, (start, end) in enumerate(chunks, start=1):
        ...
        payload = {
            "source": source,
            "previous_context": " ".join(str(item.get("text", "")) for item in segments[max(0, start - 2) : start]),
            "segments": target,
            "next_context": " ".join(str(item.get("text", "")) for item in segments[end : min(len(segments), end + 2)]),
        ...
        if not isinstance(returned, list):
            raise RuntimeError("DeepSeek 返回结果缺少 segments 数组")
...
def write_repair_outputs(
    ...
    md_path.write_text("\n\n".join(lines) + "\n", encoding="utf-8-sig")
    return [md_path, txt_path, srt_path, json_path, corrections_path]
...
def repair_transcript_file(
    ...
    repaired, corrections, usage = repair_segments(segments, source_context, client, progress)
    return write_repair_outputs(raw, repaired, corrections, usage, json_path.parent, json_path.stem, model)


def main() -> int:
    parser = argparse.ArgumentParser(description="使用 DeepSeek 校订本地转写 JSON")
    ...
    paths = repair_transcript_file(Path(args.json_file).expanduser().resolve(), api_key, args.model)
